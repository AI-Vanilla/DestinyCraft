...
public class SpaceSkyProcedure {
	@SubscribeEvent
	...
	private static void execute(@Nullable Event event, LevelAccessor world, PoseStack pose) {
		if (pose == null)
			return;
		if ((world instanceof Level _lvl ? _lvl.dimension() : Level.OVERWORLD) == (ResourceKey.create(Registries.DIMENSION, new ResourceLocation("destinycraft2:starting_lobby")))) {
			...
			if (world instanceof ClientLevel _clientLevel) {
				...
				pose.mulPose(Axis.XP.rotationDegrees(_clientLevel.getTimeOfDay(_partialTick) * 360.0F));
				Matrix4f _matrix4f = pose.last().pose();
				RandomSource _randomSource = RandomSource.create((long) 10842);
				int _amount = 1500;
				int _alpha = 255;
				_alpha = (int) (_alpha * (1.0F - _clientLevel.getRainLevel(_partialTick)));
